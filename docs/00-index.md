...
| Note | What for |
| --- | --- |
| [Why it exists](01-why.md) | The problem, the idea, and what was measured |
| [How it works today](02-how-it-works.md) | The runtime that already runs: threads, scene, shader |
| [The language — draft 0](03-language-draft-0.md) | The Spanish sketch from before it was implemented. Historical; note 09 wins |
| [Test: does the real Marea fit in the language?](04-marea-test.md) | Marea's `ExpressionController` against the language |
| [Decisions and open questions](05-decisions.md) | What was decided, and why, and what is still to be decided |
| [Three sketches for the language](06-sketches.md) | The three syntaxes that were compared. Historical. |
| [Parity with Quickshell — what is actually missing](07-whats-missing.md) | The honest inventory, by stages, and the order of work |
| [The language — the guide](09-language-v0.md) | **The language guide**: reads straight through, with the reason for every choice |
...
- ✅ **The language, v0**: a `.plm` file → a scene. Tokenizer, parser, name checking with "did you mean…?", errors with line and arrow, and **hot reload** that loses nothing. `examples/marea.plm` and `examples/face.plm` run without a line of Rust.
- ✅ **Components, `repeat` and layout** (`row`/`column`): each copy with its own names, zones and rules; each child goes to its slot with a spring. `examples/tray.plm` is a list of notices that grows and shrinks. And the file is read in four passes: the order is the reader's.
- ✅ **The logic in Luau**: a `.luau` beside the scene, in a sandbox (no `io`, 64 MB, cut off at 2 s), with facts, texts, events with a payload, timers and system commands. It hot-reloads. `tray.luau` drives a list from data.
- ✅ **The first real bar** (`examples/bar.plm` + `bar.luau`): Hyprland's workspaces, the active window, the clock and the volume, across the full width of the monitor. With it came the **system services** behind `platform/` (`sys.watch`, `sys.call`), `spawn`/`kill`, `size: full`, `screen.width`, `anchor:` in layouts and `text number(…)`.
...
- ✅ **Typing, dropping and sending from outside**: `input` (a field the renderer edits: caret, selection, clipboard), `on submit`, `on focus` / `on blur`, `on key Ctrl+k`, `on drop` for what other applications drop, the keyboard held only while it is needed, and `pleamar --say launcher "emit toggle"` so a compositor shortcut can open a scene. With it, **a launcher** (`examples/launcher.plm` + `.luau`) on top of the `apps` service.
- ✅ **Per-element renderer**: one quad per element with its box. 600 shapes cost 0.42 ms per frame against 3.66 ms for the per-pixel interpreter; 2000, 0.57 ms. With it came **ring/stroke, arc, segment, rotation** (its own and inherited), **linear gradient, border**, nested clips (up to four) and real blending between elements. And after that, closing its limitations: **affine transforms that compose** (rotation, scale, translation), **group opacity** with an intermediate layer, an exact box for rotated textures and mouse zones under transforms. All on show in `--scene showcase`.
...
- ✅ **Notifications and tray**: `notifications` (pleamar is the server; `examples/tray` shows the real ones) and `tray` (with the Telegram and ChatGPT icons drawn from their pixels). That closes B1: audio, battery, network, music, applications, workspaces, window, notifications and tray.
- ✅ **Popup surfaces** (`popup`: an `xdg_popup` showing another piece of the same scene) and **the tray menus** (`sys.ask("tray.menu", …)`, with submenus). `examples/icons` is the whole tray: the Telegram and ChatGPT icons, and their real menus.
...
- ✅ **Plugins**: a library with its `.luau` beside it, with its boundary under its own name (`Clock.now`), its own Luau state —which sees only what is its own— and its own permissions. Tested with a hostile plugin. `examples/plugins/clock` is the first one.
...
**The direction:** this is **an alternative to Quickshell**, not the engine of one particular shell. The yardstick is [Parity with Quickshell — what is actually missing](07-whats-missing.md), measured against real configurations. **Nothing in red.** After that: files and clock as services, paths (`path`), copies born at runtime (G12), windows and lock (S7), IME (E7). **Still to be tried by real hands**: popups and the tray menu (S11), the launcher with its bind (E8), the wheel (E1), notifications and tray with k4 stopped (B12, B14).
