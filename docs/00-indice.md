...
| [[pleamar · 11 Referencia del lenguaje 0.1]] | **The reference**: lexicon, EBNF grammar, every element with what it accepts, and the version number. Its examples are compiled by `./probar.sh` |
...
- ✅ **Platform boundary** (`src/plataforma/`): everything Wayland lives behind it, and `./portable.sh` checks that the core compiles for Linux, Windows and macOS. It passes.
...
- ✅ **The language, v0**: a `.plm` file → a scene. Tokenizer, parser, name checking with "did you mean…?", errors with line and arrow, and **hot reload** that loses nothing. `escenas/marea.plm` and `escenas/cara.plm` run without a line of Rust.
- ✅ **Components, `repeat` and layout** (`row`/`column`): each copy with its own names, zones and rules; each child goes to its slot with a spring. `escenas/bandeja.plm` is a list of notices that grows and shrinks. And the file is read in four passes: the order is the reader's.
...
- ✅ **The first real bar** (`escenas/barra.plm` + `barra.luau`): Hyprland's workspaces, the active window, the clock and the volume, across the full width of the monitor. With it came the **system services** behind `plataforma/` (`sys.watch`, `sys.call`), `spawn`/`kill`, `size: full`, `screen.width`, `anchor:` in layouts and `text number(…)`.
...
- ✅ **Typing, dropping and sending from outside**: `input` (a field the renderer edits: caret, selection, clipboard), `on submit`, `on focus` / `on blur`, `on key Ctrl+k`, `on drop` for what other applications drop, the keyboard held only while it is needed, and `pleamar --decir lanzador "emit toggle"` so a compositor shortcut can open a scene. With it, **a launcher** (`escenas/lanzador.plm` + `.luau`) on top of the `apps` service.
- ✅ **Per-element renderer**: one quad per element with its box. 600 shapes cost 0.42 ms per frame against 3.66 ms for the per-pixel interpreter; 2000, 0.57 ms. With it came **ring/stroke, arc, segment, rotation** (its own and inherited), **linear gradient, border**, nested clips (up to four) and real blending between elements. And after that, closing its limitations: **affine transforms that compose** (rotation, scale, translation), **group opacity** with an intermediate layer, an exact box for rotated textures and mouse zones under transforms. All on show in `--escena muestrario`.
...
- ✅ **Notifications and tray**: `notifications` (pleamar is the server; `escenas/bandeja` shows the real ones) and `tray` (with the Telegram and ChatGPT icons drawn from their pixels). That closes B1: audio, battery, network, music, applications, workspaces, window, notifications and tray.
- ✅ **Popup surfaces** (`popup`: an `xdg_popup` showing another piece of the same scene) and **the tray menus** (`sys.ask("tray.menu", …)`, with submenus). `escenas/iconos` is the whole tray: the Telegram and ChatGPT icons, and their real menus.
- ✅ **Models and `for`**: shaped data crossing the boundary (`model rows max 14 { label: text; enabled: bool = true }`, `for r in rows { Row(r) }`, `model.rows = list`). The three scenes with lists, rewritten without a slot by hand. And **the language's first test suite** (`pruebas/`, `./probar.sh`), which watches the error messages too.
...
- ✅ **Several files**: `library` and `import`, with errors that say which file, and hot reload of what was imported. `escenas/comun/` holds the palette and the menu row. The language test suite is up to 22.
- ✅ **The language, written down**: note 11 is its reference, version **0.1** (`language 0.1`, `pleamar --version`), taken from the compiler, with its examples compiled on every `./probar.sh` and its vocabulary compared against the one the compiler consults (`pleamar --gramatica`): 68 checks. Writing it uncovered two gaps, now closed: there was no `==`, and `while` only worked in two rules.
...
- ✅ **Plugins**: a library with its `.luau` beside it, with its boundary under its own name (`Clock.now`), its own Luau state —which sees only what is its own— and its own permissions. Tested with a hostile plugin. `escenas/plugins/reloj` is the first one.
- ✅ **Plugins worth trusting**: their permissions are approved by whoever uses them (`pleamar --aprobar`; unapproved, they run with none), each one on its own thread, with `require`, and they can bring their own gestures, layers and images. And the compiler checks the enums.
...
- ✅ **One surface per monitor, with its own state** (`screens: each`): `escenas/barra` is now one bar per screen, each with its own active workspace.
