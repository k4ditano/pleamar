...
Lo que queda rojo en [[pleamar · 08 Limitaciones conocidas]]: **notificaciones y bandeja** (B1: ser servidor de D-Bus), **permisos** para la lógica (U1) y los **topes fijos** del render (P7). Y por debajo, una instancia de escena por monitor (S2), el IME (E7) y que `audio` y `network` dejen de lanzar procesos (B8). **Por probar con manos de verdad**: el lanzador con su bind y el teclado exclusivo (E8).
