...
- ✅ **El lenguaje, por escrito**: la nota 11 es su referencia, versión **0.1** (`language 0.1`, `pleamar --version`), sacada del compilador y con sus ejemplos compilados en cada `./probar.sh` y su vocabulario comparado con el que consulta el compilador (`pleamar --gramatica`): 41 comprobaciones. Escribirla destapó dos huecos, ya cerrados: no había `==`, y `while` solo valía en dos reglas.
- ✅ **Componentes que dicen qué necesitan**: parámetros con tipo y valor por defecto, argumentos por nombre, y sucesos como parámetro (`MenuRow(r, chosen: choose)`). Un fallo dentro de un componente dice desde dónde se usó.
