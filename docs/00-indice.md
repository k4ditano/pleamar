...
**Nada en rojo.** El lenguaje y los plugins están hechos: modelos y `for`, textos con huecos, `import`, referencia con versión y vocabulario vigilado, componentes con contrato (tipos, huecos, `strict`), tipos comprobados, y plugins con frontera, hilo y permisos aprobados. Lo que sigue, en amarillo: copias que nazcan en marcha (G12), ventanas y bloqueo (S7), una instancia por monitor y varias escenas en un proceso (S2, G22), IME (E7), que `audio` y `network` dejen de lanzar procesos (B8), y una ventana que pregunte por los permisos (U1). **Por probar con manos de verdad**: emergentes y menú de la bandeja (S11), el lanzador con su bind (E8), la rueda (E1), notificaciones y bandeja con k4 parado (B12, B14). **Ojo al abrir `escenas/con-plugin.plm`**: el reloj sale sin hora hasta que lo apruebes con `pleamar --aprobar escenas/con-plugin.plm`.
