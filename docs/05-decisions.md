...
| 2026-09-19 | **`layer` is the central concept, not `state`** | The test against Marea: `shape` is a slot many claim, and today it is defended with eight copied guards |
...
| 2026-09-19 | **A `state` is a claim that sets properties**; there is no separate construct | Opening/closing Marea's card came out as `layer card { open while open?; rest }`, with nothing new |
...
| 2026-09-19 | **Limitations get written down as they come up, each with its fix plan** ([Known limitations](08-limitations.md)) | Abel's decision: no hidden debt |

## What implementing it taught

- **Marea's logic came down to ten lines.** Opening, closing, falling asleep, highlighting the button and the hop on press are rules. What reaches the logic is `Layer("card", "open")`, to do *its* job, and `Signal("view-event")`.
- **The island's logic decides nothing**: two layers and three rules.
- `Toggle(fact)` was needed —press to open and close— and `Away{duration}` has to "arm" itself: it only counts if the pointer was over it before, or it would close right after starting.
...
- **Pending:** two layers setting the same property (today the last one to change wins), different transitions depending on *where* it comes from, and gestures with parameters (`point(side)`).
...
| The priority order of `layer shape` | See [Test: does the real Marea fit in the language?](04-marea-test.md) §3.2 | Claude deduced it from the comments. It has to be reviewed line by line |
...
| Written or inferred types? | `fact count: int` · `fact count = 0` | |
