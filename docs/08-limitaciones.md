...
| --- | --- | --- | --- |
| B1 | **Faltan las notificaciones y la bandeja.** Ya son servicios `audio`, `battery`, `network`, `media`, `apps`, `workspaces` y `window`; esos dos no, y son de otra clase: no se escucha, **se es** el servidor de D-Bus (`org.freedesktop.Notifications`, `StatusNotifierWatcher`) | 🔴 | `zbus` ya está dentro (lo usa `media`). Un servicio `notifications` que publique el nombre en el bus y cuente `{ id, app, title, body, icon }`, con `notifications.dismiss`. **Ojo: solo uno puede tener ese nombre**, y en esta máquina lo tiene k4; para probarlo hace falta un bus de sesión aparte (`dbus-run-session`). En Windows, `UserNotificationListener`; macOS no deja leer las de otros |
