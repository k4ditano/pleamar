...
| U4 | Los hechos sueltos se leen como números (`fact.open == 1`), no como sí/no. En los modelos ya no: un campo `bool` se escribe con `true` y `false` | ⚪ | Lo mismo para los hechos: `fact open: bool = false`, y que la lógica lo lea y lo escriba como booleano |
...
| # | Limitación | Gravedad | Cómo se arregla |
| --- | --- | --- | --- |
| G12 | **Un modelo tiene un tope** (`max`, hasta 256), y sus fichas se despliegan todas al cargar: `for r in rows` con `max 14` son catorce copias, se vean o no. El lenguaje ya no lo enseña —se escribe una lista y se recorre—, pero el coste sí está: instrucciones, zonas y reglas por copia. **Sin identidad**: si se quita la primera ficha, no «sube» cada una a su sitio nuevo; cambian de contenido las que están | 🟡 | Es de la implementación, no del lenguaje: lo escrito no cambia. Que el render instancie el cuerpo del `for` por ficha (propiedades, zonas y reglas que nacen y mueren), con `key: r.id` para que cada una viaje a su sitio. Y una lista con desplazamiento, que solo instancie lo que se ve |
| G18 | Un campo es `text`, `number` o `bool`. **Sin fichas dentro de fichas** (un menú es un árbol, y hoy la lógica enseña un nivel cada vez), sin listas de números, sin `image` como tipo (se declara aparte: `image pic = from r.icon, 24, 24`) y sin enumerados (`urgency: low | normal | critical`) | 🟡 | `children: rows` para los árboles, que pide `for` anidados con emergentes hijas (S7); `image(24, 24)` como tipo, que declare la imagen sola; enumerados que en la escena se comparen por nombre (`r.urgency is critical`) y viajen como números |
...
| G22 | Una biblioteca solo declara `let`, `spring` y `component`. No puede traer **modelos, gestos, poses ni capas**, ni **imágenes** con rutas relativas a ella, ni su propio trozo de **lógica** (`.luau`): un plugin de verdad es escena + lógica + permisos, y hoy solo se comparte lo primero | 🟡 | Es U3 visto desde el lenguaje: `library` con `model` y `gesture`; rutas de `image file` relativas al fichero que las escribe; y un `.luau` por biblioteca con su propio estado y sus permisos, que el usuario aprueba |
| G23 | **Un componente no dice qué necesita de fuera.** `MenuRow` emite `choose` y lee colores: si la escena no tiene ese suceso, el fallo sale dentro de la biblioteca, no donde se usa. Tampoco hay forma de pasarle qué hacer (`on press: emit pick(r.index)`) | 🟡 | Parámetros de suceso (`component MenuRow(r, chosen: event)`) y que un fallo dentro de un componente importado añada «usado desde escena.plm:42». Va con G14 (parámetros con tipo y por defecto) |
