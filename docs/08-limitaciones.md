...
| G19 | **El lenguaje no sabe nada de textos**: ni juntar dos (`"{title} · {body}"`), ni pasarlos a mayúsculas, ni recortarlos. `bandeja` pone título y cuerpo en un `row` para no pedírselo a Luau | 🟡 | Es T8 visto desde los modelos: cadenas con huecos (`text "{r.title} · {r.body}"`) y un puñado de funciones (`upper`, `lower`, `trim`). Se resuelven en el render al cambiar cualquiera de sus partes |
