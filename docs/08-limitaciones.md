...
| G25 | **La gramática escrita y el parser son dos cosas.** La referencia (nota 11) se sacó del compilador y sus ejemplos completos se compilan en cada `./probar.sh`, pero la EBNF y las tablas de propiedades son texto: si alguien añade una propiedad y no la apunta, nada avisa | 🟡 | Que el compilador sepa contar lo que acepta (`pleamar --gramatica`: palabras, propiedades por elemento, funciones, disparadores) y que `./probar.sh` compare esa lista con la de la nota. Las tablas de `propiedades(n, &[…])` ya son datos: basta sacarlas de donde están |
