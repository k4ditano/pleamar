...
| G12 | **No se puede leer algo que se declara más abajo**: el panel del lanzador no puede usar `list.height` porque la lista se pinta después. Hoy se esquiva con una `prop` que persigue a la cuenta (`follow tall = 82 + count * 42`) | 🟡 | Declarar las propiedades de medida (`x.width`, `x.height`) en la primera vuelta, antes del dibujo: son nombres, y su valor ya llega un frame tarde de todos modos |
...
| T12 | **Una imagen no puede venir de un dato.** `image` lleva su fichero o su icono escrito en la escena; el lanzador no puede enseñar el icono de cada aplicación, que la lógica conoce (`apps` da `icon`) | 🟡 | `image hit.$i.icon` ligada a un `text`, como ya hace `text` con los textos vivos: al cambiar el nombre, el taller la pide y el render la cambia cuando llega. La caché del atlas ya va por clave |
...
| E12 | Las órdenes de fuera solo saben `emit`. No se puede preguntar nada (¿está abierto?) ni poner un hecho | ⚪ | `set hecho valor`, `get hecho`, y contestar por el mismo socket |
| E13 | La repetición de tecla es la nuestra (400 ms y 33 ms), no la que el usuario tiene puesta en su sistema | ⚪ | Leer `repeat_info` de `wl_keyboard` y sus equivalentes, y pasarla al render |
